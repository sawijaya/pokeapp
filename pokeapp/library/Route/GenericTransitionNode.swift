...
public class GenericTransitionNode<T> {
    ...
    open func then(_ block: @escaping TransitionSetupBlock<T>) throws {
        guard let destination = self.destination else { throw RSAURIRouteError.viewControllerWasNil("Destination") }
//
...
//
        if moduleInput is T {
            ...
            throw RSAURIRouteError.castError(controller: .init(describing: T.self), type: "\(moduleInput as Any)")
        }
